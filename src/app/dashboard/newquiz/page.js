  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const { data, error } = await ESG.supabase.from("institution_assesments").select("answer").eq("institution_id", userData.id).eq("que_id");
        console.log(data);
      ...
      >
        ...
        <Box sx={{ display: "flex", justifyContent: "space-between" }}>
          <Button
            ...
            sx={{ backgroundColor: "#6366F1 !important", color: "white ", borderRadius: "10px " }}
          >
            Back
          </Button>
          {questions.length > 0 && currentQuestionIndex < questions.length - 1 ? (
            currentAnswer.trim() === "" ? (
              <Button variant="contained" endIcon={<ArrowForwardIcon />} onClick={handleSkip} sx={{ backgroundColor: "#6366F1 !important", borderRadius: "10px " }}>
                Skip
              </Button>
            ) : (
              <Button variant="contained" endIcon={<ArrowForwardIcon />} onClick={handleNext} sx={{ backgroundColor: "#6366F1 !important", borderRadius: "10px " }}>
                Next
              </Button>
            )
          ) : (
            <Button
              ...
              sx={{ backgroundColor: "#6366F1 !important", borderRadius: "10px" }}
            >
              Submit
            </Button>
          )}
